---
name: 23blocks-conversations-meetings-api
description: Create and manage video/audio meetings with Vonage integration. Use when scheduling meetings, creating Vonage sessions, or managing meeting settings.
...
```bash
curl -X GET "$BLOCKS_API_URL/meetings/{unique_id}" \
...
Common status codes: `401` Unauthorized, `404` Not Found, `422` Unprocessable Entity, `503` Service Unavailable (Vonage down).
