---
name: 23blocks-onboarding-companies-api
description: Manage 23blocks onboarding companies (tenants) via REST API. Use when creating companies, managing API keys for external integrations, configuring RabbitMQ exchange settings, configuring storage, or generating impersonation tokens.
...
Complete API reference for 23blocks onboarding multi-tenant company management, API keys, exchange settings, and storage.
...
curl -X POST "$BLOCKS_API_URL/companies/company-uuid-123/keys" \
  ...
  -d '{
    "company_key": {
      "description": "Mandrill API Key",
      "provider": "mandrill",
      "api_key": "md-EXAMPLE_API_KEY",
      "api_secret": "md-EXAMPLE_SECRET",
      "api_region": "us-east-1"
    }
  }'
```
...
| `provider` | string | Yes | Provider name (mandrill, aws, etc.) |
...
Retrieves RabbitMQ exchange settings for the company.

**Request:**
```bash
curl -X GET "$BLOCKS_API_URL/companies/acme-corp/exchange" \
  -H "Authorization: Bearer $BLOCKS_AUTH_TOKEN" \
  -H "AppId: $BLOCKS_API_KEY"
```
...
Deletes exchange settings for a company.
...
Configures storage settings for the company.
...
| `bucket` | string | Yes | Storage bucket/container name |
...
**Errors:**
- `403 Forbidden` - Not authorized to configure storage
- `422 Unprocessable Entity` - Validation errors
