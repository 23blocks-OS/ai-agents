...
Retrieves company storage configuration.
...
```json
{
  "data": {
    ...
    "attributes": {
      ...
      "provider": "aws_s3",
      "bucket": "acme-products",
      "region": "us-east-1",
      "cdn_url": "https://cdn.acme.example.com",
      "max_file_size_mb": 50,
      "created_at": "2025-01-10T10:30:00Z"
    }
  }
...
Updates company storage configuration.
...
curl -X PUT "$BLOCKS_API_URL/companies/acme-corp/storage" \
  ...
  -d '{
    "storage": {
      "provider": "aws_s3",
      "bucket": "acme-products-v2",
      "region": "us-west-2",
      "cdn_url": "https://cdn-west.acme.example.com",
...
| Parameter | Type | Required | Description |
|-----------|------|----------|-------------|
| `provider` | string | Yes | Storage provider (aws_s3, gcs, azure) |
| `bucket` | string | Yes | Storage bucket name |
| `region` | string | No | Storage region |
| `cdn_url` | string | No | CDN base URL |
| `max_file_size_mb` | integer | No | Max file size in MB |

**Response 200:** Updated storage settings object
