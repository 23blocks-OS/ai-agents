---
name: 23blocks-rewards-companies-api
description: Manage 23blocks rewards companies (tenants) via REST API. Use when creating companies, managing API keys for external integrations, configuring RabbitMQ exchange settings, or generating impersonation tokens.
...
Complete API reference for 23blocks multi-tenant company management, API keys, and exchange settings for the Rewards Block.
...
Lists all API keys for external integrations (e.g., OpenAI, AWS).
...
curl -X POST "$BLOCKS_API_URL/companies/acme-corp/keys" \
  ...
  -d '{
    "company_key": {
      ...
      "api_secret": "whsec_EXAMPLE_SECRET",
      "api_region": "us-east-1"
    }
  }'
```
...
| `provider` | string | Yes | Provider name (stripe, aws, etc.) |
